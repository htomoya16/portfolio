...
export const projects: Project[] = [
  ...
  },
  {
    ...
    background:
      '複数の推しを応援する中で、ライブ、イベント、グッズ、雑誌、配信などの情報がSNS上に散らばり、見逃しや手動でのカレンダー登録が負担になる課題がありました。そこで、SNS上の情報収集からカレンダー反映までを自動化し、推し活に必要な予定を効率的に管理できるようにすることを目的に開発しました。この経験を通じて、画面に直接表れる部分だけでなく、データ取得やAPI実装など、アプリの動作を裏側から支える部分を作る面白さを実感しました。これが、バックエンド開発に強い関心を持つきっかけになりました。',
    devContent:
      'Hack U 東京電機大学 2025に5名チームで参加し、推し活情報を自動で整理・管理するカレンダーアプリを開発しました。ユーザーが推しの名前、SNSのプロフィールURL、希望する情報カテゴリなどを登録すると、関連する投稿情報を取得し、ライブや配信などの予定情報としてカレンダーに反映できる仕組みにしました。私はバックエンド担当として、イベントの種類を取得するAPIと、登録された推しに関連するイベント情報を返すAPIの実装を担当しました。Goやバックエンド開発の経験が浅い中で、データの扱い方や処理の流れを整理し、チームメンバーのコードも参考にしながら実装と動作確認を進めました。事前のユーザー調査に基づく課題設定と、推し活に特化して情報収集から予定管理までを自動化した点が評価され、東京電機大学賞を受賞しました。',
    highlights: [
  ...
  },
  {
    ...
    background:
      '仮想空間でピアノを演奏する場合、音や映像は再現できても、鍵盤に触れた感覚や押し込むときの抵抗感は再現しにくいという課題がありました。そこで、指の動きを読み取り、その動きに合わせて指先へ触覚を返すグローブ型デバイスを用いて、実際の鍵盤を押しているような感覚を再現することを目指しました。本研究は卒業研究の個人テーマとして取り組み、課題設定、システム設計、実装、評価実験まで一貫して行いました。',
    devContent:
      '卒業研究の個人テーマとして、仮想空間でピアノを演奏する際に、実際の鍵盤を押しているような感覚を再現するXR・ハプティックシステムを開発しました。ハードウェア面では、公開されている手袋型デバイスの設計データを参考に、3Dプリンタで部品を作成しました。センサ、小型モータ、ESP32マイコンを用いて回路を構成し、指の動きを取得しながら、指先へ触覚を返すグローブ型デバイスを構築しました。ソフトウェア面では、Unity上に仮想空間の手モデルとピアノを実装しました。センサから取得した指の動きに応じて手モデルを動かし、仮想空間上で鍵盤を押せるようにしました。また、鍵盤との接触状態や指の曲がり具合に応じて小型モータを制御し、押し込み感に近い触覚を提示する仕組みを実装しました。評価実験では、被験者にシステムを体験してもらい、アンケートを通じて操作感を確認しました。その結果、触覚提示によって、鍵盤を押している実感や実際のピアノに近い操作感が向上する傾向を確認しました。',
    highlights: [
  ...
  },
  {
    ...
    background:
      '対戦型ゲームを遊ぶ中で、公式サイトだけでは特定の相手との勝率や直近成績、条件別の傾向を簡単に振り返れない不便がありました。そこで、外部サービス上の対戦結果を取得・集計し、Discord上で必要な戦績をすぐに確認できるBotを開発しました。メンバー同士の対戦結果の振り返りや交流を支援するとともに、外部サービス連携、データベース設計、クラウド運用を含むバックエンド開発を実践的に学ぶことを目的としました。',
    devContent:
      'ユーザーが自身のゲームアカウントや対戦相手を登録すると、外部サービス上の対戦履歴を取得し、Discordコマンドから戦績を確認できるBotを個人開発しました。主な機能として、ユーザー情報の登録、対戦履歴の取得、履歴の一覧表示、特定の相手との対戦数・勝敗・勝率・直近成績の集計を実装しました。取得した対戦履歴はそのまま表示するのではなく、相手別の勝率や直近成績など、Discord上で利用者がすぐに確認したい情報へ整理して表示しました。また、Bot本体とデータベースをDockerで管理し、ローカル環境で必要なサービスをまとめて起動できるようにしました。さらに、Herokuへデプロイし、Discordサーバー上で継続的に利用できる構成にしました。',
    highlights: [
